...
void sigint_handler(int sig) 
{
    if (fp)
    {
        // Write trace buffer to the file

        fwrite(trace_buffer, sizeof(trace_buffer), 1, fp);

        // Close the file
        fclose(fp);
        printf("Trace file generated\n");
    }
    exit(0);
}

void tracex_init(char *filename)
{
...
CHAR    *pointer = TX_NULL;
    ...
    if (tx_trace_enable(trace_buffer, sizeof(trace_buffer), 30))
    {
        printf("Error enabling trace\n");
        exit(3);
    }
